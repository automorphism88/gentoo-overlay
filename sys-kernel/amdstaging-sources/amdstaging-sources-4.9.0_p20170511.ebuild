...
EGIT_COMMIT="dca2c8cdd7dc7f7721c8981c2618bb6f8805d31a"
KEYWORDS="~amd64"
...
src_prepare() {
	if use gentoo-base ; then
		pushd "${T}" && unpack genpatches-4.9-30.base.tar.xz && popd
	...
	if use gentoo-extras ; then
		pushd "${T}" && unpack genpatches-4.9-30.extras.tar.xz && popd
	...
	if use gentoo-experimental ; then
		pushd "${T}" && unpack genpatches-4.9-30.experimental.tar.xz && popd
