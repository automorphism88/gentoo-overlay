...
SRC_URI="gentoo-base? ( genpatches-4.9-33.base.tar.xz )
	gentoo-extras? ( genpatches-4.9-33.extras.tar.xz )
	gentoo-experimental? ( genpatches-4.9-33.experimental.tar.xz )"
...
src_prepare() {
	if use gentoo-base ; then
		pushd "${T}" && unpack genpatches-4.9-33.base.tar.xz && popd
	...
	if use gentoo-extras ; then
		pushd "${T}" && unpack genpatches-4.9-33.extras.tar.xz && popd
	...
	if use gentoo-experimental ; then
		pushd "${T}" && unpack genpatches-4.9-33.experimental.tar.xz && popd
