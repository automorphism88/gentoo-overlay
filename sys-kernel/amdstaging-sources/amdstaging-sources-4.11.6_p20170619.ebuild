...
SRC_URI="https://cdn.kernel.org/pub/linux/kernel/v4.x/patch-4.11.6.xz
	gentoo-base? ( genpatches-4.11-8.base.tar.xz )
	gentoo-extras? ( genpatches-4.11-8.extras.tar.xz )
	gentoo-experimental? ( genpatches-4.11-8.experimental.tar.xz )"

EGIT_REPO_URI="git://people.freedesktop.org/~agd5f/linux"
EGIT_BRANCH="amd-staging-4.11"

EGIT_COMMIT_DATE="2017-06-19"
KEYWORDS="~amd64"
...
src_prepare() {
	...
	if use gentoo-extras ; then
		pushd "${T}" && unpack genpatches-4.11-8.extras.tar.xz && popd
	...
	if use gentoo-experimental ; then
		pushd "${T}" && unpack genpatches-4.11-8.experimental.tar.xz && popd
