...
inherit autotools bash-completion-r1 flag-o-matic multibuild pax-utils systemd toolchain-funcs versionator
...
if [[ ${PV} != 9999 ]]; then
	if [[ ${PV} == *_alpha* || ${PV} == *_beta* || ${PV} == *_rc* ]]; then
		...
		MY_P="grub-$(get_version_component_range 1-2)"
...
QA_EXECSTACK="usr/bin/grub*-emu* usr/lib/grub/* usr/bin/grub2-* usr/sbin/grub2-* usr/lib/grub2/*"
QA_WX_LOAD="usr/lib/grub/* usr/lib/grub2/*"
QA_MULTILIB_PATHS="usr/lib/grub/.* usr/lib/grub2/.*"
...
src_prepare() {
	...
	# add missing source file if needed
	[[ -f "${S}/grub-core/tests/strtoull_test.c" ]] ||
		cp -v "${FILESDIR}/strtoull_test.c" "${S}/grub-core/tests" || die
...
src_install() {
	...
	newsbin "${FILESDIR}/opensuse-${PV}/grub2-check-default.sh" \
		grub2-check-default
