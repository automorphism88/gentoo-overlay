...
case "${PV}" in
	9999)
		...
		;;
	2.19.7)
		EGIT_COMMIT=a6447f445012c8749670405abfbc3bdcc08c9d16
		KEYWORDS="~amd64 ~x86"
		;;
	*)
		die "Unknown version"
...
src_compile() {
	exbuild /p:Platform="${EDDIE_ARCH}" src/eddie.linux.cli.sln
	use X &&
		exbuild /p:Platform="${EDDIE_ARCH}" src/eddie2.linux.ui.sln
	...
	$(tc-getCXX) \
		...
		src/App.CLI.Common.Elevated/iposix.cpp \
		src/App.CLI.Common.Elevated/ibase.cpp \
		src/App.CLI.Common.Elevated/sha256.c \
		${CXXFLAGS} ${CPPFLAGS} ${LDFLAGS} \
