...
DRIVERS_URI="http://www2.ati.com/drivers/linux/radeon-crimson-${PV}-${BUILD_VER}-151217a-297685e.zip"
...
src_prepare() {
	...
	# apply patches for 4.6+ kernels, from Arch Linux's AUR
	local -a arch_patches
	arch_patches=(
		"4.6-arch-get_user_pages_remote.patch"
	...
	for i in "${arch_patches[@]}" ; do
		epatch "${FILESDIR}/external/arch-AUR/${i}"
	done
