...
SRC_URI="https://cdn.kernel.org/pub/linux/kernel/v4.x/patch-4.11.4.xz
	gentoo-base? ( genpatches-4.11-6.base.tar.xz )
	gentoo-extras? ( genpatches-4.11-6.extras.tar.xz )
	gentoo-experimental? ( genpatches-4.11-6.experimental.tar.xz )"
...
src_prepare() {
	...
	if use gentoo-base ; then
		pushd "${T}" && unpack genpatches-4.11-6.base.tar.xz && popd
	...
	if use gentoo-extras ; then
		pushd "${T}" && unpack genpatches-4.11-6.extras.tar.xz && popd
	...
	if use gentoo-experimental ; then
		pushd "${T}" && unpack genpatches-4.11-6.experimental.tar.xz && popd
