...
SRC_URI="gentoo-base? ( genpatches-4.9-29.base.tar.xz )
	gentoo-extras? ( genpatches-4.9-29.extras.tar.xz )
	gentoo-experimental? ( genpatches-4.9-29.experimental.tar.xz )"

EGIT_REPO_URI="https://github.com/automorphism88/amd-staging-sources/"
EGIT_BRANCH="4.9-20170329"

EGIT_COMMIT="184e7e18adae3eb4ddab45e76af098cc4f327c22"
KEYWORDS="~amd64"
...
src_prepare() {
	if use gentoo-base ; then
		pushd "${T}" && unpack genpatches-4.9-29.base.tar.xz && popd
	...
	if use gentoo-extras ; then
		pushd "${T}" && unpack genpatches-4.9-29.extras.tar.xz && popd
	...
	if use gentoo-experimental ; then
		pushd "${T}" && unpack genpatches-4.9-29.experimental.tar.xz && popd
