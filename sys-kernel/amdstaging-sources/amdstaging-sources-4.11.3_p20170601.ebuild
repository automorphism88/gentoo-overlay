...
SRC_URI="https://cdn.kernel.org/pub/linux/kernel/v4.x/patch-4.11.3.xz
	gentoo-base? ( genpatches-4.11-5.base.tar.xz )
	gentoo-extras? ( genpatches-4.11-5.extras.tar.xz )
	gentoo-experimental? ( genpatches-4.11-5.experimental.tar.xz )"

EGIT_REPO_URI="git://people.freedesktop.org/~agd5f/linux"
EGIT_BRANCH="amd-staging-4.11"

EGIT_COMMIT="c6795aaac6be621964c359c42b911f108e04ecc9"
KEYWORDS="~amd64"
...
src_prepare() {
	xz -cd "${DISTDIR}"/patch-4.11.3.xz |
		filterdiff -x "*/drivers/gpu/drm/amd/amdgpu/dce_v10_0.c" -x "*/drivers/gpu/drm/amd/amdgpu/dce_v11_0.c" -x "*/drivers/gpu/drm/amd/amdgpu/dce_v6_0.c" -x "*/drivers/gpu/drm/amd/amdgpu/dce_v8_0.c" > ${T}/patch-4.11.3-fixed || die
	[[ ${PIPESTATUS[0]} == 0 ]] || die
	eapply "${T}"/patch-4.11.3-fixed

	if use gentoo-base ; then
		pushd "${T}" && unpack genpatches-4.11-5.base.tar.xz && popd
	...
	if use gentoo-extras ; then
		pushd "${T}" && unpack genpatches-4.11-5.extras.tar.xz && popd
	...
	if use gentoo-experimental ; then
		pushd "${T}" && unpack genpatches-4.11-5.experimental.tar.xz && popd
